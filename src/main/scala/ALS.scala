...
import com.sun.xml.internal.ws.encoding.MIMEPartStreamingDataHandler.MyIOException
...
object ALS {
    def main(args: Array[String]): Unit = {
        val conf = new SparkConf().setAppName("recommendMovie")
        ...
        if (args.length != 3) {
            ...
            for (i <- 1 to 10) {
                ...
                //Equation 2
                //Part A
                val ruiyi = myitemMatrix.join(ratingByItem.value).map(item => (item._2._2._1, (item._2._1.map(_ * item._2._2._2.toLong)))).reduceByKey(_ + _)
